    ...
    $result = Invoke-SPDSCCommand -Credential $InstallAccount `
        ...
        $IndexComponents = (Get-SPEnterpriseSearchComponent -SearchTopology $currentTopology `
                            | Where-Object { 
                                ...
                                -and ($_.IndexPartitionOrdinal -eq $params.Index) }).ServerName
...
function Set-TargetResource
{
    ...
    Invoke-SPDSCCommand -Credential $InstallAccount `
        ...
        if ($params.ContainsKey("RootDirectory") -eq $true)
        {
            ...
            {
                New-Item -Path $params.RootDirectory -ItemType Directory -Force
            }
        }
        ...
        {
            $AllSearchServiceInstances.Add($server,
                                           (Get-SPEnterpriseSearchServiceInstance -Identity $server))
        }
        ...
        @("Servers")  | ForEach-Object -Process { 
            ...
            else
            {
                $ComponentsToAdd = @()
                $ComponentsToRemove = @()
                foreach($Component in ($params.$CurrentSearchProperty `
                ...
                foreach($Component in ($CurrentValues.$CurrentSearchProperty `
                                       | Where-Object -FilterScript {
            ...
            foreach($ComponentToRemove in $ComponentsToRemove)
            {
                $component = Get-SPEnterpriseSearchComponent -SearchTopology $newTopology `
                             | Where-Object {($_.GetType().Name -eq $componentTypes.$CurrentSearchProperty) `
...
function Test-TargetResource
{
    ...
    Write-Verbose -Message "Testing for Search Index Partition '$Index'"
