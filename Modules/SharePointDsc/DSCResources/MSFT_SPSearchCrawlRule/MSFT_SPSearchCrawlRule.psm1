function Get-TargetResource
{
    ...
    (
        ...
        [parameter(Mandatory = $false)]
        [ValidateSet("FollowLinksNoPageCrawl","CrawlComplexUrls", "CrawlAsHTTP")]
        [System.String[]]
    ...
    if ($AuthenticationType -eq "CertificateRuleAccess" `
        -and -not $CertificateName)
    {
    ...
    if ($AuthenticationType -ne "CertificateRuleAccess" `
        -and $CertificateName)
    {
    ...
    # AuthenticationCredentials parameters, but AuthenticationType is not NTLMAccountRuleAccess or
    # BasicAccountRuleAccess
    ...
    {
        throw ("When RuleType=ExclusionRule, CrawlConfigurationRules cannot contain the values " + `
               "FollowLinksNoPageCrawl or CrawlAsHTTP")
    }
    ...
    if ($RuleType -eq "ExclusionRule" `
        -and ($AuthenticationCredentials `
            -or $CertificateName -or $AuthenticationType))
    {
        throw ("When Type=ExclusionRule, parameters AuthenticationCredentials, CertificateName " + `
               "or AuthenticationType are not allowed")
    }
    ...
    $result = Invoke-SPDSCCommand -Credential $InstallAccount `
        ...
        {
            $crawlRule = Get-SPEnterpriseSearchCrawlRule -SearchApplication $params.ServiceAppName `
                         | Where-Object { $_.Path -eq $params.Path }
            ...
            else
            {
                ...
                switch ($crawlRule.AuthenticationType)
                {
                    ...
                    { @("DefaultRuleAccess", "FormRuleAccess", "CookieRuleAccess", "AnonymousAccess") -contains $_ } {
                        $returnVal = @{
                            Path = $params.Path
                            ServiceAppName = $params.ServiceAppName
...
function Set-TargetResource
{
    ...
    if ($RuleType -eq "ExclusionRule" `
        -and ($CrawlConfigurationRules -contains "CrawlAsHTTP" `
            -or $CrawlConfigurationRules -contains "FollowLinksNoPageCrawl"))
    {
        throw ("When Type=ExclusionRule, CrawlConfigurationRules cannot contain the values " + `
               "FollowLinksNoPageCrawl or CrawlAsHTTP")
    }
...
function Test-TargetResource
{
    ...
    if ($Ensure -eq "Present")
    {
        if ($CrawlConfigurationRules)
        {
            ...
            {
                if ($null -ne (Compare-Object -ReferenceObject $CrawlConfigurationRules `
        ...
        if ($CurrentValues.ContainsKey("AuthenticationCredentials") `
            -and $AuthenticationCredentials)
        { 
