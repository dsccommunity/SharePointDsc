        ...
        foreach ($contentDatabase in $ContentDatabases)
        {
                        ...
                        -or ($contentDatabase.MembersToExclude)))
            {
                Write-Verbose -Message ("ContentDatabases: Cannot use the Members parameter " + `
                                        "together with the MembersToInclude or MembersToExclude " + `
                                        "parameters")
                return $null
            }
                ...
                -and !$contentDatabase.MembersToExclude)
            {
                Write-Verbose -Message ("ContentDatabases: At least one of the following " + `
                                        "parameters must be specified: Members, MembersToInclude, " + `
                                        "MembersToExclude")
                return $null
            }
        }
        ...
        foreach ($contentDatabase in $ContentDatabases)
        {
            if ($contentDatabase.Members `
                -and (($contentDatabase.MembersToInclude) `
                        -or ($contentDatabase.MembersToExclude)))
            {
                throw ("ContentDatabases: Cannot use the Members parameter together with the " + `
                       "MembersToInclude or MembersToExclude parameters")
            }
            ...
            if (!$contentDatabase.Members `
                ...
                -and !$contentDatabase.MembersToExclude)
            {
                throw ("ContentDatabases: At least one of the following parameters must be " + `
    ...
    $result = Invoke-SPDSCCommand -Credential $InstallAccount `
        ...
        if ($params.Members)
        {
            ...
            else
            {
                foreach ($member in $params.Members)
                {
                    ...
                    {
                        throw ("Error while setting the Shell Admin. The Shell Admin permissions " + `
                               "will not be applied. Error details: $($_.Exception.Message)")
                    }
                }
            }
        }
        ...
        if ($params.ContentDatabases)
        {
            ...
            foreach ($contentDatabase in $params.ContentDatabases)
            {
                ...
                $currentCDB = Get-SPDSCContentDatabase `
                              | Where-Object -FilterScript {
                                  $_.Name.ToLower() -eq $contentDatabase.Name.ToLower()
                                }
                    ...
                    if ($dbShellAdmins)
                    {
                        foreach ($member in $params.MembersToExclude)
                        {
                            if ($dbShellAdmins.UserName.Contains($member))
                            {
                                ...
                                {
                                    throw ("Error while removing the Shell Admin. The Shell " + `
                                           "Admin permissions will not be revoked. Error details: " + `
                                           "$($_.Exception.Message)")
                                }
                            }
                        }
...
function Test-TargetResource
{
    ...
    if ($ContentDatabases)
    {
        ...
        foreach ($contentDatabase in $ContentDatabases)
        {
            ...
            $currentCDB = $CurrentValues.ContentDatabases `
                          | Where-Object -FilterScript {
                              $_.Name.ToLower() -eq $contentDatabase.Name.ToLower()
                            }
