...
function Set-TargetResource
{
    ...
    if ($result.Ensure -eq "Absent" -and $Ensure -eq "Present") {
        ...
        Invoke-SPDSCCommand -Credential $InstallAccount `
            ...
            if ($params.ContainsKey("CloudIndex") -eq $true)
            {
                ...
                {
                    throw ("Please install SharePoint 2016 or SharePoint 2013 with August 2015 " + `
                           "CU or higher before attempting to create a cloud enabled search " + `
                           "service application")
                }
            ...
            if ($app)
            {
                ...
                if ($params.ContainsKey("SearchCenterUrl") -eq $true)
                {
                    $serviceApp = Get-SPServiceApplication -Name $params.Name `
                                  | Where-Object -FilterScript { $_.TypeName -eq "Search Service Application" }
    ...
    if ($result.Ensure -eq "Present" -and $Ensure -eq "Present")
    {
        ...
        Invoke-SPDSCCommand -Credential $InstallAccount `
            ...
            if ($params.ContainsKey("DefaultContentAccessAccount") -eq $true)
            {
                $setParams.Add("DefaultContentAccessAccountName",
                               $params.DefaultContentAccessAccount.UserName)
                $setParams.Add("DefaultContentAccessAccountPassword",
                               $params.DefaultContentAccessAccount.Password)
            } 
            Set-SPEnterpriseSearchServiceApplication @setParams
...
function Test-TargetResource
{
    ...
    if ($Ensure -eq "Present")
    {
        return Test-SPDscParameterState -CurrentValues $CurrentValues `
                                        ...
                                        -ValuesToCheck @("Ensure", "ApplicationPool", "SearchCenterUrl")
    }
    else
    {
        return Test-SPDscParameterState -CurrentValues $CurrentValues `
