function Get-TargetResource
{
    ...
    $result = Invoke-SPDSCCommand -Credential $InstallAccount `
        ...
        if ($wa.UseClaimsAuthentication -eq $true)
        {
            ...
            if ($wa.Policies.UserName -notcontains (
                    (New-SPClaimsPrincipal -Identity $params.SuperReaderAlias `
            ...
            if ($wa.Policies.UserName -notcontains (
                    (New-SPClaimsPrincipal -Identity $params.SuperUserAlias `
                                           -IdentityType WindowsSamAccountName).ToEncodedString()))
            {
                $policiesSet = $false
            }    
...
function Set-TargetResource
{
    ...
    Invoke-SPDSCCommand -Credential $InstallAccount `
        ...
        $wa = Get-SPWebApplication -Identity $params.WebAppUrl -ErrorAction SilentlyContinue
        if ($null -eq $wa) { 
            throw [Exception] ("The web applications $($params.WebAppUrl) can not be found " + `
                               "to set cache accounts")
        }
