function Get-TargetResource
{
    ...
    $result = Invoke-SPDSCCommand -Credential $InstallAccount `
        ...
        {
            # The default zone URL will change the URL of the web app, so assuming it has
            # been applied correctly then the first call there will fail as the WebAppUrl
            # parameter will no longer be the URL of the web app. So the assumption is
            # that if a matching default entry with the new public URL is found then it
            # can be returned and will pass a test.
            $aam = Get-SPAlternateURL -Zone $params.Zone `
                   | Where-Object { $_.PublicUrl -eq $params.Url } `
