...
function Set-TargetResource
{
    ...
    Invoke-SPDSCCommand -Credential $InstallAccount `
        ...
        if ($params.ContainsKey("Members") -eq $true)
        {
            ...
            foreach($desiredMember in $params.Members)
            {
                ...
                if ($CurrentValues.Members.Username -contains $desiredMember.Username)
                {
                    ...
                    if (($CurrentValues.Members `
                        | Where-Object -FilterScript { $_.Username -eq $desiredMember.Username } `
                        | Select-Object -First 1).AccessLevel -ne $desiredMember.AccessLevel)
                    {
                        # Current permission for user doesn't match what it should,
            ...
            # Iterate the current members to make sure they are in the desired list,
...
function Test-TargetResource
{
    ...
    if ($Members)
    {
        ...
        {
            Write-Verbose -Message ("Security list matches - checking that permissions match on " + `
                                    "each object")
            foreach($currentMember in $CurrentValues.Members)
            {
                if ($currentMember.AccessLevel -ne `
    ...
    if ($MembersToInclude)
    {
        ...
        foreach ($member in $MembersToInclude)
        {
            ...
            else
            {
                ...
                if ($member.AccessLevel -ne `
                        ($CurrentValues.Members | Where-Object -FilterScript {
                            $_.Username -eq $member.Username
                         } | Select-Object -First 1).AccessLevel)
                {
                    Write-Verbose -Message ("$($member.Username) has incorrect permission level. " + `
                                            "Test failed.")
                    return $false
                }
            }
