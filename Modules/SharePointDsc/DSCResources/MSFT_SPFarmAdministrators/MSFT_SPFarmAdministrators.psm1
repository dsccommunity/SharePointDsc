...
function Set-TargetResource
{
    ...
    if ($Members)
    {
        ...
        {
            Write-Verbose -Message "Farm Administrators group matches. No further processing required"
        } 
        else 
        {
            Write-Verbose -Message "Farm Administrators group does not match. Perform corrective action"
            $addUsers = @()
            $removeUsers = @()
            ...
            { 
                Write-Verbose -Message ("Adding $($addUsers.Count) users to the Farm " + `
            ...
            { 
                Write-Verbose -Message ("Removing $($removeUsers.Count) users from the Farm " + `
                                        "Administrators group")
    ...
    if ($MembersToInclude) 
    {
        ...
        foreach ($member in $MembersToInclude) 
        {
            ...
            {
                Write-Verbose -Message "$member is not a Farm Administrator. Add user to Add list"
                $addUsers += $member
            } 
            else 
            {
                Write-Verbose -Message "$member is already a Farm Administrator. Skipping"
            }
        }

        if($addUsers.count -gt 0) 
        { 
            Write-Verbose -Message "Adding $($addUsers.Count) users to the Farm Administrators group"
            $changeUsers.Add = $addUsers
            $runChange = $true
        }
    }
    ...
    if ($MembersToExclude) 
    {
        ...
        foreach ($member in $MembersToExclude) 
        {
            ...
            {
                Write-Verbose -Message "$member is a Farm Administrator. Add user to Remove list"
                $removeUsers += $member
            } 
            else 
            {
                Write-Verbose -Message "$member is not a Farm Administrator. Skipping"
            }
        }
    ...
    if ($runChange) 
    {
        Write-Verbose -Message "Apply changes"
...
function Test-TargetResource
{
    ...
    if ($Members) 
    {
        ...
        if ($null -eq $differences) 
        {
            Write-Verbose -Message "Farm Administrators group matches"
            return $true
        } 
        else 
        {
            Write-Verbose -Message "Farm Administrators group does not match"
            return $false
        }
    }
    ...
    if ($MembersToInclude) 
    {
        ...
        foreach ($member in $MembersToInclude) 
        {
            if (-not($CurrentValues.Members -contains $member)) 
            {
                Write-Verbose -Message "$member is not a Farm Administrator. Set result to false"
                $result = $false
            } 
            else 
            {
    ...
    if ($MembersToExclude) 
    {
        ...
        foreach ($member in $MembersToExclude) 
        {
            if ($CurrentValues.Members -contains $member) 
            {
                Write-Verbose -Message "$member is a Farm Administrator. Set result to false"
                $result = $false
            } 
            else 
            {
...
function Merge-SPDscFarmAdminList 
{
    ...
    $result = Invoke-SPDSCCommand -Credential $InstallAccount `
        ...
        if ($changeUsers.ContainsKey("Remove")) 
        {
            ...
            {
                $removeUser = Get-SPUser -Identity $loginName -Web $caWebapp.Url
