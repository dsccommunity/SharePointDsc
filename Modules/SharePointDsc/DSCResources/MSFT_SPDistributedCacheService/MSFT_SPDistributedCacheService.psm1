        ...
        try
        {
            ...
            $cacheHostConfig = Get-AFCacheHostConfiguration -ComputerName $computerName `
                                                            -CachePort ($cacheHost | Where-Object -FilterScript { 
                                                                $_.HostName -eq $computerName }).PortNo `
...
function Set-TargetResource
{
            ...
            Invoke-SPDSCCommand -Credential $InstallAccount `
                ...
                if($null -eq $firewallRule)
                {
                    New-NetFirewallRule -Name "SPDistCache" `
                                        -DisplayName "SharePoint Distributed Cache" `
                                        -Protocol TCP `
            ...
            Invoke-SPDSCCommand -Credential $InstallAccount `
                ...
                if ($params.ContainsKey("ServerProvisionOrder"))
                {
                    ...
                    while ($currentServer -ne $env:COMPUTERNAME)
                    {
                        ...
                        # Attempt to see if we can find the service with just the computer name,
                        # or if we need to use the FQDN
                        $si = Get-SPServiceInstance -Server $currentServer | Where-Object -FilterScript {
                        ...
                        while (($count -lt $maxCount) `
                                -and ($null -eq (Get-SPServiceInstance -Server $currentServer `
                                      | Where-Object -FilterScript { $_.TypeName -eq "Distributed Cache" `
                                                                     -and $_.Status -eq "Online" })))
                        {
                            Write-Verbose ("$([DateTime]::Now.ToShortTimeString()) - Waiting " + `
                                           "for distributed cache to start on $currentServer " + `
                                           "(waited $count of $maxCount minutes)")
                            Start-Sleep -Seconds 60
                            $count++
                        }

                        if ($null -eq (Get-SPServiceInstance -Server $currentServer `
                                       | Where-Object -FilterScript {
                                            $_.TypeName -eq "Distributed Cache" -and $_.Status -eq "Online"
                                         }))
                        {
                            Write-Warning ("Server $currentServer is not running distributed " + `
                                           "cache after waiting 30 minutes. No longer waiting " + `
                                           "for this server, progressing to next action")
                        }
                        ...
                        {
                            throw ("The server $($env:COMPUTERNAME) was not found in the array " + `
                                   "for distributed cache servers")
                        }
                        $currentServer = $params.ServerProvisionOrder[$serverCount]
                    }
                }
                ...
                Get-SPServiceInstance | Where-Object -FilterScript { $_.TypeName -eq "Distributed Cache" } `
                                      | Stop-SPServiceInstance -Confirm:$false

                $count = 0
                $maxCount = 30
                while (($count -lt $maxCount) `
                       -and ($null -ne (Get-SPServiceInstance | Where-Object -FilterScript {
                                            $_.TypeName -eq "Distributed Cache" -and $_.Status -ne "Disabled" 
                                        })))
                {
                    Write-Verbose ("$([DateTime]::Now.ToShortTimeString()) - Waiting for " + `
                                   "distributed cache to stop on all servers (waited $count " + `
                                   "of $maxCount minutes)")
                    Start-Sleep -Seconds 60
                    $count++
                }
                ...
                {
                    Write-Verbose ("$([DateTime]::Now.ToShortTimeString()) - Waiting for " + `
                                   "distributed cache to start on all servers (waited $count " + `
                                   "of $maxCount minutes)")
                    Start-Sleep -Seconds 60
                    $count++
                }
    ...
    else
    {
        ...
        Invoke-SPDSCCommand -Credential $InstallAccount `
            ...
            if ($null -eq $serviceInstance)
            {
                throw ("Unable to locate a distributed cache service instance on " + `
                       "$($env:computername) to remove")
            }               
