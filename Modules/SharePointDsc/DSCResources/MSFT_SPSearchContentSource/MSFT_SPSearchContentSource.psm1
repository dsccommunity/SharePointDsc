function Get-TargetResource
{
    ...
    $result = Invoke-SPDSCCommand -Credential $InstallAccount `
        ...
        Import-Module (Join-Path -Path $ScriptRoot `
                                 -ChildPath "..\..\Modules\SharePointDsc.Search\SPSearchContentSource.Schedules.psm1" `
                                 -Resolve)
        ...
        switch ($source.Type)
        {
            ...
            Default {
                throw ("SharePointDsc does not currently support '$($source.Type)' content sources. " + `
                       "Please use only 'SharePoint', 'FileShare' or 'Website'.")
            }
        }
        return $result     
    }
...
function Set-TargetResource
{
    ...
    switch ($ContentSourceType) {
        "SharePoint" {
            ...
            if ($ContinuousCrawl -eq $true `
                -and $PSBoundParameters.ContainsKey("IncrementalSchedule") -eq $true)
            {
    ...
    if ($ContentSourceType -ne $CurrentValues.ContentSourceType -and $Force -eq $false)
    {
        throw ("The type of the a search content source can not be changed from " + `
               "'$($CurrentValues.ContentSourceType)' to '$ContentSourceType' without deleting " + `
               "and adding it again. Specify 'Force = `$true' in order to allow DSC to do this, " + `
               "or manually remove the existing content source and re-run the configuration.")
    }

    if (($ContentSourceType -ne $CurrentValues.ContentSourceType `
        -and $Force -eq $true) `
        -or ($Ensure -eq "Absent" `
        -and $CurrentValues.Ensure -ne $Ensure))
    {
    ...
    if ($Ensure -eq "Present")
    {
        ...
        Invoke-SPDSCCommand -Credential $InstallAccount `
            ...
            if ($null -eq $source)
            {
                ...
                $source = New-SPEnterpriseSearchCrawlContentSource -SearchApplication $params.ServiceAppName `
                                                                   -Type $newType `
                                                                   -Name $params.Name `
            ...
            {
                Write-Verbose ("Content source '$($params.Name)' is not idle, stopping current " + `
                               "crawls to allow settings to be updated")
                $source = Get-SPEnterpriseSearchCrawlContentSource -SearchApplication $params.ServiceAppName `
                                                                   -Identity $params.Name
                $source.StopCrawl()
                $loopCount = 0
                ...
                while ($sourceToWait.CrawlStatus -ne "Idle" -or $loopCount > 15)
                {
                    $sourceToWait = Get-SPEnterpriseSearchCrawlContentSource -SearchApplication $params.ServiceAppName `
                                                                             -Identity $params.Name
                    Write-Verbose ("$([DateTime]::Now.ToShortTimeString()) - Waiting for " + `
                                   "content source '$($params.Name)' to be idle (waited " + `
                                   "$loopCount of 15 minutes)")
            ...
            if ($params.ContainsKey("IncrementalSchedule") -eq $true `
                -and $null -ne $params.IncrementalSchedule)
            {
                $incrementalSetArgs = @{
                ...
                switch ($params.IncrementalSchedule.ScheduleType)
                {
                    ...
                    "Weekly" { 
                        ...
                        if ((Test-SPDSCObjectHasProperty -Object $params.IncrementalSchedule `
                                                         -PropertyName "CrawlScheduleDaysOfWeek") -eq $true)
                        {
                            $OFS = ","
                            $incrementalSetArgs.Add("CrawlScheduleDaysOfWeek",
            ...
            if ($params.ContainsKey("FullSchedule") -eq $true)
            {
                ...
                switch ($params.FullSchedule.ScheduleType)
                {
                    ...
                    "Weekly" { 
                        ...
                        if ((Test-SPDSCObjectHasProperty -Object $params.FullSchedule `
                                                         -PropertyName "CrawlScheduleDaysOfWeek") -eq $true)
                        {
...
function Test-TargetResource
{
    ...
    Write-Verbose -Message "Testing for Search Content Source '$Name'"
    ...
    switch ($ContentSourceType)
    {
        "SharePoint" {
            ...
            if ($ContinuousCrawl -eq $true `
                -and $PSBoundParameters.ContainsKey("IncrementalSchedule") -eq $true)
            {
                throw ("You can not specify an incremental crawl schedule on a content " + `
                       "source that will use continous crawl")
            }
    ...
    if (($PSBoundParameters.ContainsKey("IncrementalSchedule") -eq $true) `
        ...
        -and ((Test-SPDSCSearchCrawlSchedule -CurrentSchedule $CurrentValues.IncrementalSchedule `
                                             -DesiredSchedule $IncrementalSchedule) -eq $false))
    {
        return $false
    }
    ...
    if (($PSBoundParameters.ContainsKey("FullSchedule") -eq $true) `
        ...
        -and ((Test-SPDSCSearchCrawlSchedule -CurrentSchedule $CurrentValues.FullSchedule `
                                             -DesiredSchedule $FullSchedule) -eq $false))
    {
        return $false;
    }
