function Get-TargetResource
{
    ...
    (
        ...
        [parameter(Mandatory = $false)]
        [ValidateSet("Manage Permissions",
        ...
        [parameter(Mandatory = $false)]
        [ValidateSet("Manage Personal Views",
    ...
    else
    {
        $result = Invoke-SPDSCCommand -Credential $InstallAccount `
            ...
            foreach ($lp in $params.ListPermissions)
            {
                switch ($lp)
                {
                    "Manage Lists"            { $newMask = $newMask -bor `
                                                [Microsoft.SharePoint.SPBasePermissions]::ManageLists}
                    "Override List Behaviors" { $newMask = $newMask -bor `
                                                [Microsoft.SharePoint.SPBasePermissions]::CancelCheckout}
                    "Add Items"               { $newMask = $newMask -bor `
                                                [Microsoft.SharePoint.SPBasePermissions]::AddListItems}
                    "Edit Items"              { $newMask = $newMask -bor `
                                                [Microsoft.SharePoint.SPBasePermissions]::EditListItems}
                    "Delete Items"            { $newMask = $newMask -bor `
                                                [Microsoft.SharePoint.SPBasePermissions]::DeleteListItems}
                    "View Items"              { $newMask = $newMask -bor `
                                                [Microsoft.SharePoint.SPBasePermissions]::ViewListItems}
                    "Approve Items"           { $newMask = $newMask -bor `
                                                [Microsoft.SharePoint.SPBasePermissions]::ApproveItems}
                    "Open Items"              { $newMask = $newMask -bor `
                                                [Microsoft.SharePoint.SPBasePermissions]::OpenItems}
                    "View Versions"           { $newMask = $newMask -bor `
                                                [Microsoft.SharePoint.SPBasePermissions]::ViewVersions}
                    "Delete Versions"         { $newMask = $newMask -bor `
                                                [Microsoft.SharePoint.SPBasePermissions]::DeleteVersions}
                    "Create Alerts"           { $newMask = $newMask -bor `
                                                [Microsoft.SharePoint.SPBasePermissions]::CreateAlerts}
                    "View Application Pages"  { $newMask = $newMask -bor `
                                                [Microsoft.SharePoint.SPBasePermissions]::ViewFormPages}
                }
            }
            ...
            foreach ($sp in $params.SitePermissions)
            {
                switch ($sp)
                {
                    "Manage Permissions"              { $newMask = $newMask -bor `
                                                        [Microsoft.SharePoint.SPBasePermissions]::ManagePermissions}
                    "View Web Analytics Data"         { $newMask = $newMask -bor `
                                                        [Microsoft.SharePoint.SPBasePermissions]::ViewUsageData}
                    "Create Subsites"                 { $newMask = $newMask -bor `
                                                        [Microsoft.SharePoint.SPBasePermissions]::ManageSubwebs}
                    "Manage Web Site"                 { $newMask = $newMask -bor `
                                                        [Microsoft.SharePoint.SPBasePermissions]::ManageWeb}
                    "Add and Customize Pages"         { $newMask = $newMask -bor `
                                                        [Microsoft.SharePoint.SPBasePermissions]::AddAndCustomizePages}
                    "Apply Themes and Borders"        { $newMask = $newMask -bor `
                                                        [Microsoft.SharePoint.SPBasePermissions]::ApplyThemeAndBorder}
                    "Apply Style Sheets"              { $newMask = $newMask -bor `
                                                        [Microsoft.SharePoint.SPBasePermissions]::ApplyStyleSheets}
                    "Create Groups"                   { $newMask = $newMask -bor `
                                                        [Microsoft.SharePoint.SPBasePermissions]::CreateGroups}
                    "Browse Directories"              { $newMask = $newMask -bor `
                                                        [Microsoft.SharePoint.SPBasePermissions]::BrowseDirectories}
                    "Use Self-Service Site Creation"  { $newMask = $newMask -bor `
                                                        [Microsoft.SharePoint.SPBasePermissions]::CreateSSCSite}
                    "View Pages"                      { $newMask = $newMask -bor `
                                                        [Microsoft.SharePoint.SPBasePermissions]::ViewPages}
                    "Enumerate Permissions"           { $newMask = $newMask -bor `
                                                        [Microsoft.SharePoint.SPBasePermissions]::EnumeratePermissions}
                    "Browse User Information"         { $newMask = $newMask -bor `
                                                        [Microsoft.SharePoint.SPBasePermissions]::BrowseUserInfo}
                    "Manage Alerts"                   { $newMask = $newMask -bor `
                                                        [Microsoft.SharePoint.SPBasePermissions]::ManageAlerts}
                    "Use Remote Interfaces"           { $newMask = $newMask -bor `
                                                        [Microsoft.SharePoint.SPBasePermissions]::UseRemoteAPIs}
                    "Use Client Integration Features" { $newMask = $newMask -bor `
                                                        [Microsoft.SharePoint.SPBasePermissions]::UseClientIntegration}
                    "Open"                            { $newMask = $newMask -bor `
                                                        [Microsoft.SharePoint.SPBasePermissions]::Open}
                    "Edit Personal User Information"  { $newMask = $newMask -bor `
                                                        [Microsoft.SharePoint.SPBasePermissions]::EditMyUserInfo}
                }
            }
            ...
            foreach ($pp in $params.PersonalPermissions)
            {
                switch ($pp)
                {
                    "Manage Personal Views"         { $newMask = $newMask -bor `
                                                      [Microsoft.SharePoint.SPBasePermissions]::ManagePersonalViews}
                    "Add/Remove Personal Web Parts" { $newMask = $newMask -bor `
                                                      [Microsoft.SharePoint.SPBasePermissions]::AddDelPrivateWebParts}
                    "Update Personal Web Parts"     { $newMask = $newMask -bor `
...
function Test-SPDSCInput() {
    ...
    if ($AllPermissions)
    {
        ...
        {
            throw ("Do not specify parameters ListPermissions, SitePermissions or " + `
                   "PersonalPermissions when specifying parameter AllPermissions")
        }
    }
    else
    {
...
#Checks
    ...
    if (($ListPermissions -contains "Manage Lists" `
       ...
       -and -not ($ListPermissions -contains "View Items")) {
        throw ("View Items is required when specifying Manage Lists, Override List Behaviors, " + `
               "Add Items, Edit Items, Delete Items, Approve Items, Open Items, View Versions, " + `
               "Delete Versions, Create Alerts, Manage Permissions, Manage Web Site, Add and " + `
               "Customize Pages, Manage Alerts, Use Client Integration Features, Manage Personal " + `
               "Views, Add/Remove Personal Web Parts or Update Personal Web Parts")
    }
    ...
    if (($ListPermissions -contains "Manage Lists" `
       ...
       -and -not ($SitePermissions -contains "View Pages")) {
        throw ("View Pages is required when specifying Manage Lists, Override List Behaviors, " + `
               ...
               "Apply Themes and Borders, Apply Style Sheets, Create Groups, Browse Directories, " + `
               "Use Self-Service Site Creation, Enumerate Permissions, Manage Alerts, Manage " + `
               "Personal Views, Add/Remove Personal Web Parts or Update Personal Web Parts")
    }
    ...
    {
        throw ("Browse User Information is required when specifying Manage Permissions, Create " + `
               "Subsites, Manage Web Site, Create Groups, Use Self-Service Site Creation, " + `
